...
const uninstallSrc = fs.readFileSync(path.join(__dirname, 'uninstall.js'), 'utf8');
const codexIfLine = uninstallSrc.split(/\r?\n/).find((l) => l.includes("if (agent.id === 'codex')"));
const uninstallSkillsOk =
  uninstallSrc.includes('`${name}-gag`') &&
  !!codexIfLine &&
  /^ {6}/.test(codexIfLine) &&
  !/^ {8}/.test(codexIfLine);
if (uninstallSkillsOk) ok('uninstaller cleans Codex *-gag skills (independent of the command dir)');
else bad('uninstaller skill cleanup', 'must use -gag naming and run independently of the command dir');
...
const markerCases = [
  ...
  ['Auto-generated by @lqmnwido/global-ai-agents v1.4.7', true],
];
